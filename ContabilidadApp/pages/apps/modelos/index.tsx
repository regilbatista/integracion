...
import SaveModelosModal from '@/components/pages/modelos/saveModelosModal';
...
const Modelos = () => {
    ...
    useEffect(() => {
       ...
       const fentchModels = async ()  =>{
        const emp = await apiGet({ path: 'modelos' });
        ...
        const [saveParams, setSaveParams] = useState<any>(JSON.parse(JSON.stringify(modelsDefault)));
        const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
        const [initialRecords, setInitialRecords] = useState<any>([]);
        const [recordsData, setRecordsData] = useState<any>(initialRecords);
        const [addModelsModal,setAddModelsModal] = useState<any> (false);
            ...
            setRecordsData(() => {
                return initialRecords.filter((item: any) => {
                    return (
                        ...
                        item.Marca?.descripcion?.toString().toLowerCase().includes(search.toLowerCase())
                    );
                });
            });
        }, [search]);
        ...
        const addEditData = (row: any = null) => {
            ...
            setAddModelsModal(true);
        };

        const deleteItem = async (row: any) => {
            const resp = await apiDelete({ path: 'modelos', data: saveParams, id: row.id });
            let message = {};
            ...
            <>
                <div className="panel">
                    <div className="mb-5 flex flex-col gap-5 md:flex-row md:items-center">
                        <h5 className="text-lg font-semibold dark:text-white-light">Modelos</h5>
                ...
                <SaveModelosModal
                    ...
                    fentchModels={fentchModels}
                    modelsDefault={modelsDefault}
                />
            </>
        );
