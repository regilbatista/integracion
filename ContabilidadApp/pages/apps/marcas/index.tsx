...
import SaveMarcasModal from '@/components/pages/marcas/saveMarcasModal';
...
const Marcas = () => {
    ...
    useEffect(() => {
       ...
       const fentchBrand = async ()  =>{
        const emp = await apiGet({ path: 'marcas' });
        ...
        const [saveParams, setSaveParams] = useState<any>(JSON.parse(JSON.stringify(brandDefault)));
        const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
        const [initialRecords, setInitialRecords] = useState<any>([]);
        const [recordsData, setRecordsData] = useState<any>(initialRecords);
        const [addBrandModal,setAddBrandModal] = useState<any> (false);
        ...
        const deleteItem = async (row: any) => {
            const resp = await apiDelete({ path: 'marcas', data: saveParams, id: row.id });
            let message = {};
            ...
            <>
                <div className="panel">
                    <div className="mb-5 flex flex-col gap-5 md:flex-row md:items-center">
                        <h5 className="text-lg font-semibold dark:text-white-light">Marcas</h5>
