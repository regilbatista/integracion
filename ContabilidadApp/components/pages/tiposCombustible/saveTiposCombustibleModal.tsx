...
const selectDefault: {
    ...
    const SaveTiposCombustibleModal = (props: any) => {
        const { addTypesFuelModal, setAddTypesFuelModal, setSaveParams, saveParams, fentchTypesFuel, typesfuelDefault } = props;
           ...
           try {
            saveParams.estado_Id=1;
                ...
                if (saveParams.id) {
                    ...
                    const resp = await apiPatch({ path: 'tiposcombustible', data: saveParams, id });
                    ...
                    // Insert task
                    const resp = await apiPost({ path: 'tiposcombustible', data: saveParams });
        ...
        const close = () => {
            setErrors([]);
            setAddTypesFuelModal(false);
            setSaveParams(typesfuelDefault);
        }


    return (
        <Transition appear show={addTypesFuelModal} as={Fragment}>
            <Dialog
                as="div"
                open={addTypesFuelModal}
                onClose={() => {
                    close();
                }}
                            ...
                            >
                                <div className="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                                    <div className="bg-[#fbfbfb] py-3 text-lg font-medium ltr:pl-5 ltr:pr-[50px] rtl:pl-[50px] rtl:pr-5 dark:bg-[#121c2c]">
                                        {saveParams?.id ? 'Editar tipos de combustible' : 'Añadir tipos de combustible'}
                                    </div>
                                    <button
                                        type="button"
                                        onClick={() => {
...
export default SaveTiposCombustibleModal;
